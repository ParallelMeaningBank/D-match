...
import os, random, time, argparse, re, sys
reload(sys)
...
def get_memory_usage():
	...
	mem = process.get_memory_info()[0] / float(1000000)
	return mem


def get_best_match(instance1, attribute1, relation1,
				   instance2, attribute2, relation2,
				   prefix1, prefix2, var_count, var_types1, var_types2, var_map_prod, var_map_gold, prin, single, significant, memory_limit, counter, doattribute=True, dorelation=True):
	"""
	Get the highest triple match number between two sets of triples via hill-climbing.
	Arguments:
		instance1: instance triples of DRG 1 ("instance", node name, node value)
		...
		instance2: instance triples of DRG 2 ("instance", node name, node value)
	...
	(candidate_mappings, weight_dict) = compute_pool(instance1, attribute1, relation1,
													 instance2, attribute2, relation2,
													 prefix1, prefix2, var_count, var_types1, var_types2, var_map_prod, var_map_gold, doattribute=doattribute, dorelation=dorelation)
	...
	total_triples 	= len(instance1) + len(attribute1) + len(relation1)
	found_idx 		= 0
	random_mappings = []
	all_fscores 	= []
	...
	for i, map_cur in enumerate(mapping_order):   #number of restarts is number of mappings
		...
		while True:
			# get best gain
			(gain, new_mapping) = get_best_gain(cur_mapping, candidate_mappings, weight_dict, len(instance2), match_num, match_triple_dict)
...
def compute_pool(instance1, attribute1, relation1,
				 instance2, attribute2, relation2,
				 prefix1, prefix2, var_count, var_types1, var_types2, var_map_prod, var_map_gold, doattribute=True, dorelation=True):
	"""
	...
	Arguments:
		instance1: instance triples of DRG 1
		attribute1: attribute triples of DRG 1 (attribute name, node name, attribute value)
		relation1: relation triples of DRG 1 (relation name, node 1 name, node 2 name)
		instance2: instance triples of DRG 2
		attribute2: attribute triples of DRG 2 (attribute name, node name, attribute value)
		relation2: relation triples of DRG 2 (relation name, node 1 name, node 2 name
	...
	if dorelation:
		#for i in range(0, len(relation1)): 	#for only relations we need to add candidate space here #Rik removed this
		#	candidate_mapping.append(set())
				...
				if normalize(relation1[i][0]) == normalize(relation2[j][0]):
					if var_types1[relation1[i][1]] == var_types2[relation2[j][1]] and var_types1[relation1[i][2]] == var_types2[relation2[j][2]]: 	#check if nodes are even allowed to match, based on their type
						...
						if node_pair2 != node_pair1:
							...
							if node1_index_drg1 > node2_index_drg1:
...
def smart_att_mapping(candidate_mapping, attribute1, attribute2):
	...
	Arguments:
		...
		attribute1: instance triples of DRG 1
		attribute2: instance triples of DRG 2
	Returns:
	...
	for i, candidates in enumerate(candidate_mapping):
		if len(candidates) == 0:
			...
			result.append(-1)
			continue
		# node value in instance triples of DRG 1
			...
			if int(rel[1][1:]) == i and len(re.findall(r'\.[a-z]\.[\d]+', rel[0])) > 0:
				...
				for node_index in candidates:
					for rel2 in relation2:
						if int(rel2[1][1:]) == node_index:	#relation exists for this node index
...
def get_best_gain(mapping, candidate_mappings, weight_dict, instance_len, cur_match_num, match_triple_dict):
	"""
	...
	instance_len: the number of the nodes in DRG 2
...
def print_alignment(mapping, instance1, instance2):
	...
	Args:
		...
		instance1: nodes of DRG 1
		instance2: nodes of DRG 2
...
def create_tab_list(print_rows):
...
def get_matching_triples(arg_list):
	...
	prod_t, gold_t, args, prefix1, prefix2, dorelation, doattribute, significant, prin, prod_file, gold_file, single, ms, smart, restarts, max_triples, memory_limit, counter = arg_list #unpack argument list
	...
	instance1 = []	#instances are empty for DRGs
	instance2 = []
	...
	if verbose:
		...
		print >> DEBUG_LOG, "============================================"
		print >> DEBUG_LOG, "Instance triples of DRG 1:", len(instance1)
		...
		print >> DEBUG_LOG, "Instance triples of DRG 2:", len(instance2)
	...
	(best_mapping, best_match_num, found_idx, restarts_done, avg_f, smart_fscores) = get_best_match(instance1, attributes1, relation1,
													instance2, attributes2, relation2,
													prefix1, prefix2, var_count1, var_types1, var_types2, var_map_prod, var_map_gold, prin, single, significant, memory_limit, counter, doattribute=doattribute, dorelation=dorelation)
...
def print_results(res_list, no_print, start_time, prin, ms, smart, restarts, significant, max_triples, single):
	...
	if not res_list:
		...
		if prin:	#print specific output here
			...
			if smart == 'all':
				print 'Smart attribute F-score      : {0}'.format(compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
				print 'Smart order F-score          : {0}'.format(compute_f(sum([y[1] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
				print 'Smart F-score initial matches: {0}'.format(compute_f(sum([y[2] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
				print 'Smart F-score concepts       : {0}'.format(compute_f(sum([y[3] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
			elif smart == 'attr':
				print 'Smart attribute F-score: {0}'.format(compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
			elif smart == 'order':		
				print 'Smart order F-score: {0}'.format(compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
			elif smart == 'init':
				print 'Smart F-score initial matches: {0}'.format(compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
			elif smart == 'conc':
				print 'Smart F-score concepts: {0}'.format(compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num, significant, True))
			
			if single:	#averaging does not make for multiple DRGs	
				print 'Average F-score over all restarts: {0}'.format(avg_f)
...
def main(arguments):
	...
	if args.baseline:		#if we try a baseline DRG, we have to fill a list of this baseline
		...
		raise ValueError("Erorr: Number of DRGs not equal, {0} vs {1}".format(len(triples_prod_list), len(triples_gold_list)))
	elif len(triples_prod_list) == 0 and len(triples_gold_list) == 0:
		raise ValueError("Both DRGs empty")
	else:
		print 'Comparing {0} DRGs...\n'.format(len(triples_gold_list))	
	...
	for idx in range(args.runs):	#for experiments we want to more runs so we can average later
		...
		for prod_t, gold_t in zip(triples_prod_list, triples_gold_list):		
			counter += 1
			arg_list.append([prod_t, gold_t, args, prefix1, prefix2, dorelation, doattribute, args.significant, args.prin, args.f1, args.f2, single, args.ms, args.smart, args.restarts, args.max_triples, args.mem_limit, counter])
